"""
run_benchmark.py (AttentiveFP)
-------------------------------
Runs AttentiveFP (DGL_AttentiveFP via DeepPurpose) across all 22 TDC ADMET tasks.
...
# ── AttentiveFP runner with tuning ─────────────────────────────────────────────

def run_attentivefp(train, valid, test, task_name, logs_dir):
    """
    Fine-tunes DGL_AttentiveFP with hyperparameter search on validation set.
    ...
    with open(os.path.join(logs_dir, f"{task_name}_tuning.json"), "w") as f:
        json.dump({
            "task": task_name, "model": "AttentiveFP",
...
def run_benchmark(n_runs=5, task_filter=None):
    ...
    print(f"{'='*60}")
    print(f"  Model:   AttentiveFP (DGL_AttentiveFP)")
    ...
    for task_name in benchmark_names:
        ...
        try:
            ...
            for run_idx in range(n_runs):
                ...
                preds, final_model, best_params = run_attentivefp(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AttentiveFP TDC ADMET Benchmarking Script")
